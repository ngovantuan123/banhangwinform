...
namespace DoAn_2.MenuTab
{
	...
	{
		Invoice invoice;
		List<InvoiceDetail> lstIvcDlt;

		InvoiceDAO ivcDao = new InvoiceDAO();
		InvoiceDetailDAO ivcdtlDao = new InvoiceDetailDAO();
		ProductDAO prdDao = new ProductDAO();
		ComboDAO cbDao = new ComboDAO();

		// init
		public HoaDonChiTiet(int id)
		{
			InitializeComponent();

			invoice = ivcDao.getByID(id);
			if (invoice == null)
			{
			...
			lstIvcDlt = ivcdtlDao.getAllofInvoice(id);

			// update
			updateTable();

			// set status
			cbbStatus.SelectedIndex = invoice.status;
		}
		...
		private void updateTable()
		{
			...
			lstIvcDlt.ForEach(i => {
				...
				Product prd = prdDao.getByID(i.Product_ID);
				Combo cb = null;
				if (i.Combo_ID != null)
					cb = cbDao.getByID((int)i.Combo_ID);

				datas[0] = prd.Product_Name; // san pham
				datas[1] = cb == null ? "" : cb.Combo_Name; // combo
				datas[2] = i.Amount.ToString();
				datas[3] = i.Price.ToString();
				dgvCTDH.Rows.Add(datas);
			...
			if (dgvCTDH.Rows.Count > 0)
			{
				...
				if (sfd.ShowDialog() == DialogResult.OK)
				{
					...
					if (!fileError)
					{
						try
						{
							PdfPTable pdfTable = new PdfPTable(dgvCTDH.Columns.Count);
		...
		{
			if (dgvCTDH.CurrentCell.RowIndex == -1)
				return;

			nbrAmount.Value = lstIvcDlt[dgvCTDH.CurrentCell.RowIndex].Amount;
		}

		private void btnChange_Click(object sender, EventArgs e)
		{
			// Xac nhan
			DialogResult dialogResult = MessageBox.Show("Lưu thay đổi", "Xác nhận", MessageBoxButtons.YesNo);
			...
			// Lay so luong
			int amount = Convert.ToInt32(nbrAmount.Value);
			if (amount > 0)
			{
				int price = lstIvcDlt[dgvCTDH.CurrentCell.RowIndex].Price;
				int oamount = lstIvcDlt[dgvCTDH.CurrentCell.RowIndex].Amount;
				lstIvcDlt[dgvCTDH.CurrentCell.RowIndex].Amount = amount;
				lstIvcDlt[dgvCTDH.CurrentCell.RowIndex].Price = price / oamount * amount;
			}

			// Luu database
			ivcDao.update(invoice);
			//ivcdtlDao.update(lstIvcDlt[dgvCTDH.CurrentCell.RowIndex]); // TODO ERROR !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
